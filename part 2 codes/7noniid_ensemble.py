...
#This script creates a harder and more realistic federated setting by making the clients non-IID. 
# That means the client datasets no longer follow the same distribution. Some clients are made larger or smaller, and their dominant transportation modes differ.
...
clients_dir = r"C:\Users\user\Desktop\VIP\federated\results\part2_kaust_local\noniid_clients"
save_root = r"C:\Users\user\Desktop\VIP\federated\results\part2_kaust_local\noniid_federated_ensemble"
...
print("Loading all non-IID client datasets...")
all_clients = []
...
print("Shared non-IID test set size:", len(X_test_shared))
...
for file in client_files:
    ...
    # train on full local client data
    model = Pipeline([
    ...
    metrics["local_rows"] = int(len(X_client))
...
plt.title("Non-IID Federated-Style Ensemble Confusion Matrix")
plt.colorbar()
...
plt.title("Non-IID Client Models vs Ensemble - Macro F1")
plt.xlabel("Model")
plt.ylabel("Macro F1")
